document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(event) {
        ...
        if (inputPassword[0].value === inputPassword[1].value && inputPassword[0].value.length >= 6) {
            // Exibe um popup de sucesso
            showSuccessPopup();
            // Aqui você pode adicionar o código para enviar a senha para o servidor
        } else {
            // Senhas não são iguais ou são curtas
            alert('Por favor, certifique-se de que as senhas são iguais e têm pelo menos 6 caracteres.');
        }
    });
